...
public abstract class RenderCrop extends RenderBase {
    ...
    public boolean render(RenderBlocks render, IBlockAccess world, int x, int y, int z) {
        this.isGiant = HarvestMoon.handler.getClient().getCropTracker().isCropGiant(ClientHelper.getWorld(), x, y, z);
        this.stage = HarvestMoon.handler.getClient().getCropTracker().getCropStage(ClientHelper.getWorld(), x, y, z);
