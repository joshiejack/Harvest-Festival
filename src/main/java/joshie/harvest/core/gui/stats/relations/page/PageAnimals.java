...
import joshie.harvest.core.gui.stats.relations.button.ButtonRelationsAnimal;
...
public class PageAnimals extends PageStats {
    ...
    public void initGui(GuiStats gui, List<GuiButton> buttonList, List<GuiLabel> labelList) {
        ...
        while (it.hasNext()) {
            ...
            if (EntityHelper.getStats(animal) == null || EntityHelper.getStats(animal).getOwner() != MCClientHelper.getPlayer()) {
                it.remove();
            }
        }
        ...
        for (int i = start * 12; i < 12 + start * 12 && i < list.size(); i++) {
            ...
            buttonList.add(new ButtonRelationsAnimal(gui, animal, EntityHelper.getStats(animal), buttonList.size(), 16 + x * 144, 25 + y * 25));
            y++;
