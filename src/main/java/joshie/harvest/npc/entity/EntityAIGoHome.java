...
public class EntityAIGoHome extends EntityAIBase {
    ...
    public void startExecuting() {
        if (this.entity.getDistanceSq(home) > 256.0D) {
            Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards(this.entity, 14, 3, new Vec3d((double) home.getX() + 0.5D, (double) home.getY(), (double) home.getZ() + 0.5D));
            if (vec3d != null) {
                this.entity.getNavigator().tryMoveToXYZ(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord, 1.0D);
            }
        } else {
            this.entity.getNavigator().tryMoveToXYZ((double) home.getX() + 0.5D, (double) home.getY(), (double) home.getZ() + 0.5D, 1.0D);
        }
    }
