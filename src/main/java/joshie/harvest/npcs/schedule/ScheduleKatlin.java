...
public class ScheduleKatlin implements ISchedule {
    @Override
    public BuildingLocation getTarget(World world, EntityLiving entity, NPC npc, Season season, Weekday day, long time) {
        if (time >= 5000L && time <= 7000L) return GODDESS_POND_FRONT_LEFT;
        else if (time >= 7000L && time <= 17000L) return day == SUNDAY ? CHURCHPEWCENTRE : CAFEFRONT;
        else if (time >= 17000L && time <= 19000L) return FISHING_POND_RIGHT;
