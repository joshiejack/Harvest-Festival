...
public class ScheduleYulife implements ISchedule {
    ...
    public BuildingLocation getTarget(World world, EntityLiving entity, NPC npc, Season season, Weekday day, long time) {
        if (time >= 9000L && time <= 17000L) return CARPENTER_FRONT;
        else if (time >= 22000L && time <= 24000L) return CARPENTER_UPSTAIRS;
        else if (time >= 0L && time <= 5000L) return CARPENTER_UPSTAIRS;
        else return CARPENTER_DOWNSTAIRS;
    }
}
