                ...
                for (int j = 0; j < method.instructions.size(); j++) {
                    ...
                    if (instruction.getType() == AbstractInsnNode.LDC_INSN) {
                        ...
                        if (ldcInstruction.cst.equals("snow_layer")) {
                            ((TypeInsnNode) method.instructions.get(j + 1)).desc = HMModInfo.ASMPATH + "asm/overrides/BlockSnowSheet";
                            ((MethodInsnNode) method.instructions.get(j + 3)).owner = HMModInfo.ASMPATH + "asm/overrides/BlockSnowSheet";
                            ((MethodInsnNode) method.instructions.get(j + 5)).owner = HMModInfo.ASMPATH + "asm/overrides/BlockSnowSheet";
                            break topLabel;
                        }
                    }
                }
