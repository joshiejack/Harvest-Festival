...
public class RenderCookware implements ISimpleBlockRenderingHandler {
    ...
    public RenderCookware() {
        ItemBlockCookware item = (ItemBlockCookware) Item.getItemFromBlock(HMBlocks.cookware);
        for (int i = 1; i < 8; i++) {
            String name = item.getName(new ItemStack(item, 1, i)).replace(".", "_");
            try {
                models.put(i, AdvancedModelLoader.loadModel(new ResourceLocation(HMModInfo.MODPATH, "models/" + name + ".obj")));
                resources.put(i, new ResourceLocation(HMModInfo.MODPATH, "textures/models/" + name + ".png"));
            } catch (Exception e) {}
        }
    }
