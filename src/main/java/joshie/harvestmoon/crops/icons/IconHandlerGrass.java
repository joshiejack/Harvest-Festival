...
public class IconHandlerGrass extends AbstractIconHandler {
    ...
    public void registerIcons(IIconRegister register) {
        ...
        for (int i = 0; i < bottomIcons.length; i++) {
            bottomIcons[i] = register.registerIcon(HMModInfo.CROPPATH + HMCrops.grass.getUnlocalizedName() + "_bottom_" + (i + 1));
        }

        topIcons = new IIcon[6];
        for (int i = 0; i < topIcons.length; i++) {
            topIcons[i] = register.registerIcon(HMModInfo.CROPPATH + HMCrops.grass.getUnlocalizedName() + "_top_" + (i + 1));
        }
    }
}
