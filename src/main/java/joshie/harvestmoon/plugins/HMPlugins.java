...
public class HMPlugins {
    ...
    private static Plugin get(PluginData p) {
        try {
            ...
            HarvestMoon.logger.log(Level.INFO, "Failed to load plugin for modid: " + p.name);
        }
    ...
    public static void loadConfigs() {
        Configuration config = new Configuration(new File(HarvestMoon.root, "/plugins.cfg"));
        for (PluginData p : data) {
            if (Loader.isModLoaded(p.name)) { //If the mod is loaded
                try {
                    ...
                    HarvestMoon.logger.log(Level.ERROR, HMModInfo.MODNAME + " failed to load in it's " + p.name + " config");
                    e.printStackTrace();
                } finally {
                    config.save();
                }
