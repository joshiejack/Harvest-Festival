...
import static joshie.harvestmoon.core.helpers.QuestHelper.rewardItem;
import static joshie.harvestmoon.core.helpers.QuestHelper.takeHeldStack;
import joshie.harvestmoon.api.npc.INPC;
import joshie.harvestmoon.core.helpers.generic.OreDictionaryHelper;
...
public class QuestGoddess extends Quest {   
    ...
    protected INPC[] getNPCs() {
        return new INPC[] { HMNPCs.goddess, HMNPCs.seed_owner };
    }
    ...
    public void onStageChanged(EntityPlayer player, int previous, int stage) {
        ...
        if (previous == 0) {
            rewardItem(player, new ItemStack(HMBlocks.flowers, 4, 0));
        } else if (previous == 1) {
            rewardItem(player, HMBuildings.carpenter.getPreview());
            rewardItem(player, new ItemStack(Blocks.red_flower, 1, player.worldObj.rand.nextInt(8)));
        } else if (previous == 3) {
            ItemStack seeds = HMCrops.turnip.getSeedStack().copy();
            seeds.stackSize = 10;
