...
import uk.joshiejack.gastronomy.item.ItemUtensil;
import uk.joshiejack.gastronomy.tile.TileMixer;
...
@PenguinLoader(value = "mixer", side = Side.CLIENT)
public class SpecialRendererMixer extends SpecialRendererCookware<TileMixer> {
    ...
    private ItemStack getBladeStack() {
        if (BLADE_STACK.isEmpty()) {
            BLADE_STACK = GastronomyItems.UTENSIL.getStackFromEnum(ItemUtensil.Utensil.BLADE);
        }
