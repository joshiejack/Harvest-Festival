...
public class PacketAcceptCitizenship extends PenguinPacket {
    private UUID uuid;
    private int dimension;
    private int id;

    public PacketAcceptCitizenship() {}
    public PacketAcceptCitizenship(int dimension, int id, UUID uuid) {
    ...
    public void handlePacket(EntityPlayer player) {
        ...
        if (PenguinTeams.getTeamForPlayer(player).getID().equals(town.getCharter().getTeamID())) {
            PenguinTeams.get(player.world).changeTeam(player.world, PlayerHelper.getUUIDForPlayer(player), town.getCharter().getTeamID());
